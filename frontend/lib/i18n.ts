...
const translations = {
  en: {
    ...
    home: {
      heroTitle: "Turn Your Kid's Drawing Into a Tesla Wrap",
      heroSubtitle:
        "Print the template, let your child color it, take a photo, and get a custom Tesla wrap file in seconds.",
      steps: ["Print & Color", "Upload Photo", "Download PNG", "Apply to Tesla"],
      step1Header: "1. Select Your Tesla Model",
      step2Header: "2. Download & Print Template",
      step3Header: "3. Upload Your Photo",
      downloadTemplate: (model: string) => `Download ${model} Template (PDF)`,
      templateHint:
  ...
  ko: {
    ...
    home: {
      heroTitle: "아이의 그림을 테슬라 랩으로 만들어보세요",
      heroSubtitle:
        "템플릿을 출력하고, 아이가 색칠하고, 사진을 찍으면 테슬라 커스텀 랩 파일이 완성됩니다.",
      steps: ["출력 & 색칠", "사진 업로드", "PNG 다운로드", "테슬라에 적용"],
      step1Header: "1. 테슬라 모델 선택",
      step2Header: "2. 템플릿 다운로드 & 출력",
      step3Header: "3. 사진 업로드",
      downloadTemplate: (model: string) => `${model} 템플릿 다운로드 (PDF)`,
      templateHint: "A4 또는 Letter 용지에 출력하세요. 아이가 크레용이나 마커로 색칠하도록 해주세요.",
